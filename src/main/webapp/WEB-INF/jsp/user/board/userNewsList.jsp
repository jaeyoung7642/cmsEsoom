...
function fn_egov_select_linkPage(pageNo){
	...
	document.newsForm.action = "<c:url value='/user/selectNewsList.do'/>";
					...
					<article class="board_list">
						...
						<div class="board_header">
							...
							<div class="search_box">
								...
								<form name="newsForm" action="<c:url value='/user/selectNewsList.do'/>" method="post" onSubmit="fn_egov_search_news(); return false;" class="search_box_form frm_group">
									<select name="searchCondition" class="frm_select" data-forms="select" aria-label="검색조건 선택">
										<option value="0"  <c:if test="${searchVO.searchCondition == '0'}">selected="selected"</c:if> >뉴스제목</option><!-- 뉴스제목 -->
										<option value="1"  <c:if test="${searchVO.searchCondition == '1'}">selected="selected"</c:if> >뉴스출처</option><!-- 뉴스출처 -->
									</select>
									<input type="text" name="searchKeyword" class="frm_input" aria-label="검색어 입력" value="<c:out value="${searchVO.searchKeyword}"/>">
							...
							<li class="row">
								...
								<a href="#" class="bbs_link">
									...
									<div class="bbs_cont">
										...
										<p class="cont txt_line3 txt_16-14">${resultInfo.newsCn}</p>
									</div>
									<div class="bbs_info">
										<p class="el_ico_bg date">${resultInfo.frstRegisterPnttm}</p>
										<p class="el_ico_bg view">${resultInfo.newsOrigin}</p>
									</div>
								</a>
							</li>
							</c:forEach>
