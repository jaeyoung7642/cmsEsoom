		...
		<main id="container" class="ly_container">
			...
			<div class="ly_inner sub_content_wrap">
				...
				<section id="content" class="ly_content">
					...
					<article class="page_guide bl_box gray">
						<div class="row">
							<div class="icon">
								<span class="el_ico ico_guide_01 ccl"></span>
							</div>
							<div class="col">
								<ul class="desc_list_dot">
									<li>글 작성시 개인정보(주민등록번호, 핸드폰 번호 등) 입력에 주의하시기 바랍니다.</li>
									<li>특정개인 및 단체에 대한 비방, 상업성 광고, 욕설, 정치적 성향 내재 등의 글은 [이천시 인터넷시스템 설치 및 운영에 관한 조례 제6조]에 의거 사전 통보 없이 삭제될 수 있습니다.
										<ul class="desc_list_dash">
											<li>영리목적의 상업성 광고, 저작권을 침해할 수 있는 내용</li>
											<li>욕설, 음란물 등 불건전한 내용</li>
											<li>동일인 또는 동일인이라고 인정되는 사용자가 동일한 게시물을 반복적으로 게시하는 경우 등</li>
										</ul>
									</li>
									<li class="wrap">자유게시판은 "답변을 하지 않는 게시판"이오니 답변을 원하실 경우 민원신청을 이용하여 주시기바랍니다. <a href="#" target="_blank" class="el_btn btn3 line_point w160"><c-icon name="link2" size="14" class="lt"></c-icon>민원신청 바로가기</a>
									</li>
								</ul>
							</div>
						</div>
					...
					<article class="board_list">
						...
						<table class="bbs_list_tbl">
							...
							<tbody>
								...
								<tr>
									<td class="num"><c:out value="${(searchVO.pageIndex-1) * searchVO.pageUnit + status.count}"/></td>
									<td class="tit">
										<a href="<c:url value='/user/selectArticleDetail.do?bbsId=${resultInfo.bbsId}&nttId=${resultInfo.nttId}'/>" class="tit_link">
											<span class="tit_txt txt_line1-2">
												${resultInfo.nttSj }
											</span>
										</a>
									</td>
									<td class="name info"><c:out value='${resultInfo.frstRegisterNm}'/></td>
									<td class="date info"><c:out value='${resultInfo.frstRegisterPnttm}'/></td>
									<td class="num"><c:out value='${resultInfo.inqireCo}'/></td>
								</tr>
								</c:forEach>
								<!-- (게시물 없을 경우) -->
					...
					<div class="btn_area mt30 p_end">
						<a href="<c:url value='/user/insertArticleView.do?bbsId=${boardMasterVO.bbsId}' />" class="el_btn btn2 point w140">글쓰기</a>
					</div>
