...
<script type="text/javascript">
	...
	function fn_egov_downFile(thumFileId, fileSn){
		window.open("<c:url value='/cmm/fms/FileDown.do?thumFileId="+thumFileId+"&fileSn="+fileSn+"'/>");
	}

	function fn_egov_deleteFile(thumFileId, fileSn, delTrName) {
		var forms;
		var form;
		forms = document.getElementsByTagName("form");

		for (var i = 0; i < forms.length; i++) {
			if (typeof(forms[i].thumFileId) != "undefined" &&
		...
		var newthumFileId = document.createElement( 'input' );
		console.log(newfileSn);
		...
		newthumFileId.setAttribute("name","thumFileId");
		
		newfileSn.setAttribute("type","hidden");
		newthumFileId.setAttribute("type","hidden");

		newfileSn.setAttribute("value",fileSn);
		newthumFileId.setAttribute("value",thumFileId);
		
		newForm.appendChild( newfileSn );
		newForm.appendChild( newthumFileId );

		//form = document.forms[0];
		//form.thumFileId.value = thumFileId;
...
<input type="hidden" name="thumFileId" value="<c:out value='${thumFileId}'/>">
		...
		<tr id="egov_file_view_table_tr_${status.count}">
			...
			<c:choose>
				<c:when test="${updateFlag eq 'Y'}">
					...
					<img src="<c:url value='/images/egovframework/com/cmm/btn/btn_del.png' />" class="cursor" onClick="fn_egov_deleteFile('<c:out value="${thumFileId}"/>','<c:out value="${fileVO.fileSn}"/>','egov_file_view_table_tr_${status.count}');" alt="<spring:message code="title.attachedFileDelete" />">
				</c:when>
				<c:otherwise>
					<a href="javascript:fn_egov_downFile('<c:out value="${fileVO.thumFileId}"/>','<c:out value="${fileVO.fileSn}"/>')">
					<c:out value="${fileVO.orignlFileNm}"/>&nbsp;[<c:out value="${fileVO.fileMg}"/>&nbsp;byte]
					</a>
				</c:otherwise>
