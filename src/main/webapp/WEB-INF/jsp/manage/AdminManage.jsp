...
<c:set var="pageTitle">관리자 관리</c:set>
<!DOCTYPE html>
...
function fnSelectUser(id) {
    ...
    document.listForm.action = "<c:url value='/AdminUpdtView.do'/>";
    document.listForm.submit();

}
function fnAddUserView() {
    document.listForm.action = "<c:url value='/adminInsertView.do'/>";
    document.listForm.submit();
}
function fnLinkPage(pageNo){
    document.listForm.pageIndex.value = pageNo;
    document.listForm.action = "<c:url value='/adminUserManage.do'/>";
    document.listForm.submit();
}
function fnSearch(){
		...
		<div class="page-content">
			<div class="container-fluid">
				...
				<div class="row">
					<div class="col-lg-12">
						<div class="card">
							<form name="listForm" action="<c:url value='/adminUserManage.do'/>" method="post"> 
							<div class="card-header">
								<div class="row justify-content-between align-items-center g-3">
									<div class="col-xxl-auto">
										<p class="text-muted mb-0 fw-medium">사용자수 <span class="badge bg-primary-subtle text-primary fs-12 ms-1"><c:out value="${paginationInfo.totalRecordCount}"/></span></p>
									</div>
									<!--end col-->
									<div class="col-xxl-auto">
										<div class="d-flex flex-column flex-xl-row gap-2">
											...
											<div class="d-flex gap-2 flex-grow-1">
												...
												<a href="<c:url value='/adminInsertView.do'/>" class="btn btn-primary w-xs" onClick="fnAddUserView(); return false;"><i class="ri-add-line align-bottom me-1"></i> 등록</a>
			...
			<table class="table align-middle table-nowrap mb-0">
				...
				<tbody>
					...
					<tr>
						<td>
							<div class="form-check">
								<input name="checkField" class="form-check-input" title="checkField <c:out value="${status.count}"/>" type="checkbox"/>
								<input name="checkId" type="hidden" value="<c:out value='${result.userTy}'/>:<c:out value='${result.uniqId}'/>"/>
							</div>
						</td>
						<td><c:out value="${status.count}"/></td>
						<td><a href="<c:url value='/AdminUpdtView.do'/>?selectedId=<c:out value="${result.uniqId}"/>"  onclick="javascript:fnSelectUser('<c:out value="${result.userTy}"/>:<c:out value="${result.uniqId}"/>'); return false;"><c:out value="${result.userId}"/></a></td>
						...
						<td>
							<c:forEach var="emplyrSttusCode_result" items="${emplyrSttusCode_result}" varStatus="status">
								<c:if test="${result.sttus == emplyrSttusCode_result.code}">
									<c:if test="${result.sttus == 'P'}">
									<span class="badge bg-success-subtle text-info fw-medium fs-12"><c:out value="${emplyrSttusCode_result.codeNm}"/></span>
									</c:if>
									<c:if test="${result.sttus == 'D'}">
									<span class="badge bg-info-subtle text-info fw-medium fs-12"><c:out value="${emplyrSttusCode_result.codeNm}"/></span>
									</c:if>
									<c:if test="${result.sttus == 'A'}">
									<span class="badge bg-danger-subtle text-info fw-medium fs-12"><c:out value="${emplyrSttusCode_result.codeNm}"/></span>
									</c:if>
								</c:if>
							</c:forEach>
						</td>
