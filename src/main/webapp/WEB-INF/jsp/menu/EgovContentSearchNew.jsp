...
function linkPage(pageNo){
	...
	document.progrmManageForm.action = "<c:url value='/sym/prm/EgovContentSearchNew.do'/>";
...
<body>
<form name="progrmManageForm" action ="<c:url value='/sym/prm/EgovContentSearchNew.do'/>" method="post">
				...
				<div class="card-header">
					<div class="row justify-content-between g-3">
						<div class="d-flex justify-content-sm-end align-items-center gap-2">
							...
							<span>페이지명 : </span>
							<div class="col">
								<input class="form-control form-control-sm" name="searchKeyword" type="text"  value='<c:out value="${searchVO.searchKeyword}"/>'  size="30" maxlength="60" title="<spring:message code="title.searchCondition"/>" /><!-- 검색조건 -->
							</div>
							...
							<tbody>
								<c:forEach var="result" items="${resultList}" varStatus="status">
			  					<tr>
			    					...
			    					<td>
			      						<a href="#LINK" onclick="choisProgramListSearch('<c:out value="${result.conCode}"/>','<c:url value='/user/selectContentsDetail.do?conCode=${result.conCode}' />'); return false;"><c:out value="${result.conTtl}"/></a>
			    					</td>
			  					</tr>
			 					</c:forEach>
							</tbody>
