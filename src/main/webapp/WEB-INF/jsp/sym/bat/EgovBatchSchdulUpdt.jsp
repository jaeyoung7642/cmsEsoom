...
<c:set var="pageTitle"><spring:message code="comSymBat.batchSchdulUpdt.title"/></c:set>
<!DOCTYPE html>
<html lang="ko" data-layout="vertical" data-topbar="light" data-sidebar="dark" data-sidebar-size="lg" data-sidebar-image="none" data-preloader="disable" data-theme="default" data-theme-colors="default">
<head>
<c:import url="/_include/head.jsp" />
<meta http-equiv="content-type" content="text/html; charset=utf-8" >
<title><spring:message code="comSymBat.batchSchdulUpdt.title"/></title><!-- 배치스케줄 수정 -->
...
function fn_egov_save(){
    var varForm = document.getElementById("batchSchdul");
    varForm.action =  "<c:url value='/sym/bat/updateBatchSchdul.do'/>";
    ...
    if (varForm.executCycle.value == "02") {
      ...
      if (i == varForm.executSchdulDfkSes.length) {
        alert("<spring:message code="comSymBat.batchSchdulUpdt.validate.executSchdulDfkSes.length"/>"); //실행주기를 매주로 선택했을 때 요일은 필수 입력값입니다.
        return ;
      }
    }
    ...
    if (varForm.executCycle.value == "03") {
        if (varForm.executSchdulDay.value == "") {
            alert("<spring:message code="comSymBat.batchSchdulUpdt.validate.cycleMonth.executSchdulDay"/>"); //실행주기가 매월일때 실행스케줄(일)은 필수 입력값입니다.
             return ;
        }
    }
    if (varForm.executCycle.value == "04") {
        if (varForm.executSchdulMonth.value == "")   {
            alert("<spring:message code="comSymBat.batchSchdulUpdt.validate.cycleYear.executSchdulMonth"/>"); //실행주기가 매년일때 실행스케줄(월)은 필수 입력값입니다.
             return ;
        }
        if (varForm.executSchdulDay.value == "") {
            alert("<spring:message code="comSymBat.batchSchdulUpdt.validate.cycleYear.executSchdulDay"/>"); //실행주기가 매년일때 실행스케줄(일)은 필수 입력값입니다.
             return ;
        }
    ...
    if ( varForm.executCycle.value == "05" )  {
        ...
        if (varForm.executSchdulDeNm.value == "")   {
            alert("<spring:message code="comSymBat.batchSchdulUpdt.validate.executSchdulDeNm"/>"); //실행주기가 한번만일때 실행스케줄(일자)는 필수 입력값입니다.
             return ;
        }
        if (!isDate(varForm.executSchdulDeNm.value, "<spring:message code="comSymBat.batchSchdulUpdt.validate.executSchdulDeNm.isDate"/>")) { //실행스케줄(일자)가 유효하지 않습니다.
             return ;
        }
    }
...
<c:set var="vexecutSchdulHour"><spring:message code="comSymBat.batchSchdulUpdt.executSchdulHour"/></c:set>
<c:set var="vexecutSchdulMnt"><spring:message code="comSymBat.batchSchdulUpdt.executSchdulMnt"/></c:set>
<c:set var="vexecutSchdulSecnd"><spring:message code="comSymBat.batchSchdulUpdt.executSchdulSecnd"/></c:set>
					...
					<div class="col-lg-12">
						<div class="card">
							<div class="card-header">
								<form:form modelAttribute="batchSchdul" id="batchSchdul" action="/sym/bat/updateBatchSchdul.do" method="post">
...
<div class="wTableFrm">
	<!-- 타이틀 -->
	<h2><spring:message code="comSymBat.batchSchdulUpdt.pageTop.title"/></h2><!-- 배치스케줄 수정 -->
	...
	<table class="wTable">
		...
		<tr>
			<th><spring:message code="comSymBat.batchSchdulUpdt.batchSchdulId"/> <span class="pilsu">*</span></th><!-- 배치스케줄ID -->
			<td class="left">
				<form:input path="batchSchdulId" size="20" maxlength="20" readonly="true"  cssClass="readOnlyClass" cssStyle="width:150px"/>
            	<form:errors path="batchSchdulId" cssClass="error" />
			</td>
		</tr>
		<tr>
			<th><spring:message code="comSymBat.batchSchdulUpdt.batchOpertId"/> <span class="pilsu">*</span></th><!-- 배치작업ID -->
			<td class="left">
				<form:input path="batchOpertId" size="20" maxlength="20" readonly="true"  cssClass="readOnlyClass" cssStyle="width:150px"/>
	            <form:errors path="batchOpertId" cssClass="error" />
	            <a href="#LINK" onclick="fn_egov_popup_batch_opert_list(); return false;" style="selector-dummy: expression(this.hideFocus=false);"><img src="<c:url value='/images/egovframework/com/cmm/icon/search2.gif' />" alt="배치작업조회팝업 제공"/></a>
			</td>
		</tr>
		<tr>
			<th><spring:message code="comSymBat.batchSchdulUpdt.batchOpertNm"/> <span class="pilsu">*</span></th><!-- 배치작업명 -->
			<td class="left">
		...
		<tr>
			<th><spring:message code="comSymBat.batchSchdulUpdt.executCycleNm"/> <span class="pilsu">*</span></th><!-- 실행주기 -->
			<td class="left">
	            ...
	            <input name="executSchdulDeNm" id="executSchdulDeNm" type="text" maxlength="10" title="<spring:message code="comSymBat.batchSchdulUpdt.executSchdulDeNm"/>" readonly style="width:70px" /><!-- 실행스케줄(일자) -->
	            </span>
	            <span id="spnMonth">
	                <select name="executSchdulMonth" id="executSchdulMonth" title="<spring:message code="comSymBat.batchSchdulUpdt.executSchdulMonth"/>"><!-- 실행스케줄(월) -->
				...
				<spring:message code="comSymBat.batchSchdulUpdt.spnMonth"/><!-- 월 -->
	              </span>
	              <span id="spnDay">
	                <select name="executSchdulDay" id="executSchdulDay" title="<spring:message code="comSymBat.batchSchdulUpdt.executSchdulDay"/>"><!-- 실행스케줄(일) -->
	                    <option value="" selected="selected"><spring:message code="input.cSelect"/></option><!-- 선택 -->
				...
				<spring:message code="comSymBat.batchSchdulUpdt.spnDay"/><!-- 일 -->
	            </span><br />
	            <span id="spnDfk">
	            ...
	            <form:errors path="executSchdulHour" cssClass="error"/>
	              <spring:message code="comSymBat.batchSchdulUpdt.spnHH"/><!-- 시 -->
	            ...
	            <form:errors path="executSchdulMnt" cssClass="error"/>
	              <spring:message code="comSymBat.batchSchdulUpdt.spnMM"/><!-- 분 -->
	            ...
	            <form:errors path="executSchdulSecnd" cssClass="error"/>
	              <spring:message code="comSymBat.batchSchdulUpdt.spnSS"/><!-- 초 -->
	            </span>
			</td>
		</tr>
	</table>
	...
	<div class="btn">
		...
		<span class="btn_s"><a href="<c:url value='/sym/bat/getBatchSchdulList.do'></c:url>" onclick="fn_egov_list(); return false;"><spring:message code="button.list" /></a></span>
	</div>
