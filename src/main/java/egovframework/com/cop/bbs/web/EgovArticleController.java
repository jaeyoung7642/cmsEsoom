...
public class EgovArticleController {
    ...
    public String selectArticleList(@ModelAttribute("searchVO") BoardVO boardVO, ModelMap model) throws Exception {
		...
		if(boardVO.getBbsId() != null && !"".equals(boardVO.getBbsId())) {
			...
			master = (BoardMasterVO) egovBBSMasterService.selectNotUsedBdMstrList(vo);
		...
		Map<String, Object> map2 = egovBBSMasterService.allBBSMasterInfs(vo);
		
		//----------------------------
		// 2025.04.09 만족도조사 추가
		//----------------------------
		//if (bbsSatisfactionService.canUseSatisfaction(master.getBbsId())) {
		if (master.getStsfdgAt().equals("Y")) {
		    model.addAttribute("useSatisfaction", "true");
		}
		System.out.print("<<<<<<<<<"+master);
		
		model.addAttribute("bbsNmList", map2.get("resultList"));
		...
		model.addAttribute("bbsIdParam", bbsIdParam);
		
		if(master.getBbsTyCode().equals("BBST03")){
			return "board/EgovNoticeList";
		} else if (master.getBbsTyCode().equals("BBST02")){
			return "board/EgovMediaList";
		} else {
    ...
    public String selectArticleDetail(@ModelAttribute("searchVO") BoardVO boardVO, ModelMap model) throws Exception {
		...
		*/
		...
		if (masterVo.getStsfdgAt().equals("Y")) {
		    model.addAttribute("useSatisfaction", "true");
		}
		////--------------------------
		...
		return "board/EgovArticleDetail";
    }
    ...
    public String insertArticleView(@ModelAttribute("searchVO") BoardVO boardVO, ModelMap model) throws Exception {
		...
		return "board/EgovArticleRegist";
    }
    ...
    public String insertArticle(final MultipartHttpServletRequest multiRequest, @ModelAttribute("searchVO") BoardVO boardVO,
		...
		//파일아이디 암호화/복호화 우회시키기 위해 임시조치 2025.04.16 esoom
		board.setAtchFileId(board.getAttachFileId());
...
//		    final Map<String, MultipartFile> files = multiRequest.getFileMap();
		    ...
		    if (!files.isEmpty()) {
		    	result = fileUtil.parseFileInf(files2, "BBS_", 0, "", "",folderPath);
		    	atchFileId2 = fileMngService.insertFileInfs(result);
		    }
		    board.setThumFileId(atchFileId2);
		    */
    ...
    public String replyBoardArticle(final MultipartHttpServletRequest multiRequest, @ModelAttribute("searchVO") BoardVO boardVO,
		...
		if(!isAuthenticated) {	//KISA 보안취약점 조치 (2018-12-10, 이정은)
		    ...
		    String folderPath = "bbs" + bbsId.substring(bbsId.length()-3);
		    if (!files.isEmpty()) {
				List<FileVO> result = fileUtil.parseFileInf(files, "BBS_", 0, "", "",folderPath);
				atchFileId = fileMngService.insertFileInfs(result);
		    }
    ...
    public String updateBoardArticle(final MultipartHttpServletRequest multiRequest, @ModelAttribute("searchVO") BoardVO boardVO,
		...
		System.out.println("@@@@@@@@@@@@@@@@@@@@@@@@@"+thumFileId);
		String decodedAtchFileId = "";
		if (thumFileId != null && !"".equals(thumFileId) ) {
			decodedAtchFileId = cryptoService.decrypt(thumFileId);
		}
		beanValidator.validate(board, bindingResult);
		...
		if (isAuthenticated) {
		    ...
		    final List<MultipartFile> files2 = multiRequest.getFiles("file_2");
		    if (!files2.isEmpty()) {
		    	if (decodedAtchFileId == null || "".equals(decodedAtchFileId)) {
		    		List<FileVO> result = fileUtil.parseFileInf(files2, "BBS_", 0, decodedAtchFileId, "",folderPath);
		    		decodedAtchFileId = fileMngService.insertFileInfs(result);
		    		board.setThumFileId(decodedAtchFileId);
		    	} else {
		    		FileVO fvo = new FileVO();
		    		fvo.setAtchFileId(decodedAtchFileId);
		    		int cnt = fileMngService.getMaxFileSN(fvo);
		    		List<FileVO> _result = fileUtil.parseFileInf(files2, "BBS_", cnt, decodedAtchFileId, "",folderPath);
		    		fileMngService.updateFileInfs(_result);
		    		board.setThumFileId(decodedAtchFileId);
		    	}
		    }
...
//		//비밀글은 작성자만 볼수 있음 
//		if(!EgovStringUtil.isEmpty(vo.getSecretAt()) && vo.getSecretAt().equals("Y") && !((user == null || user.getUniqId() == null) ? "" : user.getUniqId()).equals(vo.getFrstRegisterId()))
//		mav.setViewName("redirect:/cop/bbs/selectArticleList.do");
//		return mav;
//    }
//    
//    /**
...
//			mav.setViewName ("redirect:/cop/cmt/updateArticleCommentView.do");
//		}
//		
