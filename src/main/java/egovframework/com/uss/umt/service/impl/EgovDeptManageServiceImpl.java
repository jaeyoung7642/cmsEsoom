...
public class EgovDeptManageServiceImpl extends EgovAbstractServiceImpl implements EgovDeptManageService {
	...
	@Override
	public List<DeptManageVO> selectOrganChart(String orgnztTy) throws Exception {
		List<DeptManageVO> chartList = deptManageDAO.selectOrganChart(orgnztTy);
		for (DeptManageVO chart : chartList) {
			List<DeptManageVO> childList = deptManageDAO.selectOrganChild(chart.getOrgnztId());
			chart.setChildList(childList);
			System.out.print(">>>>>>>>>" + chart);
		}
		return chartList;
		//return deptManageDAO.selectOrganChart(deptManageVO);
	}
	@Override
	public List<DeptManageVO> selectOrganChild(String parentId) throws Exception {
		return deptManageDAO.selectOrganChild(parentId);
	}
}
